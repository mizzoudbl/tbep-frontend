...
interface PromptInputEditableProps extends Omit<HTMLAttributes<HTMLDivElement>, 'onChange'> {
  onChange?: (value: string) => void;
  placeholder?: string;
  className?: string;
}

export const PromptInputEditable = ({
  onChange,
  className,
  placeholder = 'What would you like to know?',
  ...props
}: PromptInputEditableProps) => {
  const divRef = useRef<HTMLDivElement>(null);
  const [isEmpty, setIsEmpty] = useState(true);

  // Keep placeholder visibility
  useEffect(() => {
    if (divRef.current) {
  ...
  const handleKeyDown: KeyboardEventHandler<HTMLDivElement> = e => {
    if (e.key === 'Enter') {
      ...
      if (e.shiftKey) {
        // Let contentEditable insert newline
        return;
      }
  ...
  return (
    // biome-ignore lint/a11y/useFocusableInteractive: bla bla
    // biome-ignore lint/a11y/useSemanticElements: bla bla
    <div
      ref={divRef}
      contentEditable
      role='textbox'
      ...
      className={cn(
        'max-h-48 min-h-10 w-full overflow-y-auto px-3 py-2',
